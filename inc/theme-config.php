...
EOT;
    ...
    // 导航栏深色模式 logo
    $form->addInput(new Typecho_Widget_Helper_Form_Element_Text('navDarkLogoUrl', null, null, _t('深色模式 Logo 图片地址'), _t('主题包含浅色和深色模式，如果您希望浅色模式和深色模式显示不同的 Logo，可以在这里填写深色模式的 Logo 图片 URL。如果需要深色模式和浅色模式都使用同一个 Logo 图片，可以只填写上方的 <b style="color: #C7254E;">站点 Logo 图片地址</b>，这里可以留空。')));
    ...
    $form->addInput(new Typecho_Widget_Helper_Form_Element_Radio('shortcode', array(
        'enable' => '启用',
        'disable' => '禁用'
    ), 'enable', _t('短代码'), _t('短代码是 Facile 主题提供的一种特殊语法，通过短代码可以插入一些特殊样式的内容和小组件，访问 <a href="https://facile.misterma.com/%E7%9F%AD%E4%BB%A3%E7%A0%81.html" target="_blank">帮助文档</a> 可以查看详细的短代码使用说明。如果您需要使用其它短代码插件，可以选择禁用主题的短代码功能。')));
    // MathJax 支持
